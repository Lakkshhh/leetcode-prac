class Solution:
    def findMedianSortedArrays(self, nums1: list[int], nums2: list[int]) -> float:
        ...
        low, high = 0, len(short) - 1
        while True:
            mid_short = (low + high) // 2
            mid_long = half_length - mid_short - 2

            left_short = short[mid_short] if mid_short >= 0 else float("-infinity")
            right_short = short[mid_short + 1] if (mid_short + 1) < len(short) else float("infinity")
            left_long = long[mid_long] if mid_long >= 0 else float("-infinity")
            right_long = long[mid_long + 1] if (mid_long + 1) < len(long) else float("infinity")

            if left_short <= right_long and left_long <= right_short:
                if total_length % 2:
                    return min(right_short, right_long)
                return (max(left_short, left_long) + min(right_short, right_long)) / 2
            elif left_short > right_long:
                high = mid_short - 1
            else:
                low = mid_short + 1
